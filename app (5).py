...
.card{background:#f0f8f2;border-radius:10px;padding:1rem 1.4rem;
       margin-bottom:.8rem;color:#1a1a1a !important}
.theory-box{background:#eafaf1;border-left:5px solid #27ae60;
            border-radius:6px;padding:1rem 1.4rem;margin:.6rem 0;
            font-size:.93rem;color:#1a1a1a !important}
...
.ok-box{background:#eafaf1;border-left:5px solid #27ae60;
        border-radius:6px;padding:.9rem 1.3rem;margin:.6rem 0;
        font-weight:600;color:#1e8449 !important}
.err-box{background:#fdedec;border-left:5px solid #e74c3c;
         border-radius:6px;padding:.9rem 1.3rem;margin:.6rem 0;
         font-weight:600;color:#922b21 !important}
.section-hdr{color:#1a5e36 !important;font-size:1.05rem;font-weight:600;
...
.interp{background:#f8f9fa;border-radius:8px;padding:.9rem 1.3rem;
        font-size:.9rem;color:#333 !important;margin-top:.5rem}
.theory-box *,.warn-box *,.card *,.interp *{color:inherit !important}
.ok-box *{color:#1e8449 !important}
.err-box *{color:#922b21 !important}
/* Ecuaciones/codigo dentro de cajas custom — siempre legible */
.theory-box code,.warn-box code,.card code,.interp code{
    background:rgba(39,174,96,0.18) !important;
    color:#1a3d2b !important;
    padding:2px 6px;border-radius:3px;font-size:.88em}
/* Tablas dentro de cajas custom */
.theory-box table,.warn-box table{
    color:#1a1a1a !important;font-size:.85rem;width:100%}
.theory-box th{background:#27ae60 !important;color:#fff !important;
               padding:5px 8px}
.theory-box td{color:#1a1a1a !important;padding:4px 8px;
               border-bottom:1px solid #a9dfbf}
/* Texto de main-title y sub-title — adaptable a ambos temas */
.main-title{font-size:2rem;font-weight:700;color:#1a5e36 !important;margin-bottom:0}
.sub-title{font-size:.95rem;color:#555 !important;margin-bottom:1.2rem}
</style>
...
if _logo_b64:
    _img_style = "height:64px;width:auto;object-fit:contain;filter:brightness(0) invert(1);"
    _logo_html = '<img src="data:image/png;base64,' + _logo_b64 + '" style="' + _img_style + '">'
else:
    _logo_html = '<span style="font-size:2.6rem">🧪</span>'
...
with st.sidebar:
    # Logo en sidebar
    import base64 as _b64s, os as _oss
    _sb_logo = ""
    if _oss.path.exists("assets/logo_ica.png"):
        with open("assets/logo_ica.png", "rb") as _sf:
            _sb_logo = _b64s.b64encode(_sf.read()).decode()
    if _sb_logo:
        _sb_html = '<img src="data:image/png;base64,' + _sb_logo + '" style="max-width:100%;height:auto;margin-bottom:6px;">'
...
if pagina == "🏠 Inicio e Instrucciones":
    ...
    with c1:
        ...
        <span style="background:rgba(39,174,96,0.2);color:#1a3d2b;padding:2px 7px;border-radius:4px;font-family:monospace;font-size:.88em">x_ij = μ + α_i + ε_ij</span><br><br>
        donde <span style="background:rgba(39,174,96,0.2);color:#1a3d2b;padding:2px 7px;border-radius:4px;font-family:monospace;font-size:.88em">μ</span> es la media global, <span style="background:rgba(39,174,96,0.2);color:#1a3d2b;padding:2px 7px;border-radius:4px;font-family:monospace;font-size:.88em">α_i</span> el efecto de la unidad <i>i</i>,
        y <span style="background:rgba(39,174,96,0.2);color:#1a3d2b;padding:2px 7px;border-radius:4px;font-family:monospace;font-size:.88em">ε_ij ~ N(0, σ²)</span> el error aleatorio.<br><br>
        <b>Tabla ANOVA:</b><br>
        <table style="width:100%;font-size:.85rem;border-collapse:collapse">
        <tr>
          <th style="background:#27ae60;color:#fff;padding:5px 8px;text-align:left">Fuente</th>
          <th style="background:#27ae60;color:#fff;padding:5px 8px;text-align:left">SC</th>
          <th style="background:#27ae60;color:#fff;padding:5px 8px;text-align:left">gl</th>
          <th style="background:#27ae60;color:#fff;padding:5px 8px;text-align:left">CM</th>
        </tr>
        <tr>
          <td style="color:#1a1a1a;padding:4px 8px;border-bottom:1px solid #a9dfbf">Entre unidades</td>
          <td style="color:#1a1a1a;padding:4px 8px;border-bottom:1px solid #a9dfbf">m·&Sigma;(x&#772;&#7522; &minus; x&#772;)&sup2;</td>
          <td style="color:#1a1a1a;padding:4px 8px;border-bottom:1px solid #a9dfbf">n&minus;1</td>
          <td style="color:#1a1a1a;padding:4px 8px;border-bottom:1px solid #a9dfbf">CM_entre</td>
        </tr>
        <tr>
          <td style="color:#1a1a1a;padding:4px 8px">Residual</td>
          <td style="color:#1a1a1a;padding:4px 8px">&Sigma;&Sigma;(x&#7522;&#8326; &minus; x&#772;&#7522;)&sup2;</td>
          <td style="color:#1a1a1a;padding:4px 8px">n(m&minus;1)</td>
          <td style="color:#1a1a1a;padding:4px 8px">CM_res</td>
        </tr>
        </table>
        </div>""", unsafe_allow_html=True)
    ...
    with c2:
        ...
        conservadora: <span style="background:rgba(39,174,96,0.2);color:#1a3d2b;padding:2px 7px;border-radius:4px;font-family:monospace;font-size:.88em">u_hom = √(CM_res / m)</span><br><br>
        <b>Si F_calc > F_crit (p ≤ α) → HETEROGÉNEO</b><br>
        Existe variabilidad significativa entre unidades. Se cuantifica:
        <span style="background:rgba(39,174,96,0.2);color:#1a3d2b;padding:2px 7px;border-radius:4px;font-family:monospace;font-size:.88em">u_hom = √(s²_bb)</span> donde <span style="background:rgba(39,174,96,0.2);color:#1a3d2b;padding:2px 7px;border-radius:4px;font-family:monospace;font-size:.88em">s²_bb = (CM_entre − CM_res)/m</span><br><br>
        ...
        st.markdown("### Analisis de residuales del modelo")
        st.markdown("""<div class="theory-box">
        La validez del ANOVA requiere verificar los <b>supuestos del modelo</b>.
        Los graficos y pruebas se presentan en el siguiente orden:<br><br>
        <b>Fila superior:</b><br>
        <b>1. Residuales vs valores ajustados:</b> Dispersión aleatoria en torno a cero
        indica que el modelo es adecuado y no hay patrones sistemáticos.<br>
        <b>2. Distribución de residuales (histograma):</b> Debe aproximarse a una
        forma de campana simétrica para confirmar normalidad.<br><br>
        <b>Fila inferior:</b><br>
        <b>3. Prueba de Shapiro-Wilk + Grafico Q-Q:</b> Prueba formal de normalidad
        (H₀: residuales normales). Si p > α: supuesto cumplido. El grafico Q-Q
        complementa visualmente: los puntos deben seguir la linea diagonal roja.<br>
        <b>4. Prueba de Bartlett:</b> Homocedasticidad — igualdad de varianzas entre
        unidades (H₀: σ²₁ = ... = σ²ₙ). Si p > α: varianzas homogeneas.
        </div>""", unsafe_allow_html=True)
...
"""
    ...
    # ── Fila A: Residuales vs ajustados | Histograma ──────────
    ...
    fig_h = go.Figure()
    fig_h.add_trace(go.Histogram(x=r['residuales'],
        nbinsx=min(15, len(r['residuales'])//2+1),
        marker_color="#8e44ad", opacity=0.75))
    fig_h.update_layout(title="Distribucion de residuales",
                        xaxis_title="Residual", yaxis_title="Frecuencia",
                        height=340, margin=dict(t=50,b=50,l=60,r=20), showlegend=False)
    rg2.plotly_chart(fig_h, use_container_width=True)

    # ── Fila B: [Shapiro-Wilk + Q-Q] | [Bartlett + nota] ──────
    st.markdown("---")
    col_sw, col_bt = st.columns(2)

    with col_sw:
        st.markdown("**Prueba de Shapiro-Wilk** (normalidad de residuales)")
        css_sw = "ok-box" if sw_ok else "err-box"
        msg_sw = ("Residuales normales (no se rechaza H0)"
                  if sw_ok else "No normalidad detectada (se rechaza H0)")
        st.markdown(
            '<div class="' + css_sw + '">'
            'W = ' + f'{r["sw_stat"]:.5f}' + ' | p = ' + f'{r["sw_p"]:.5f}' +
            ' | ' + msg_sw + '</div>',
            unsafe_allow_html=True)
        # Q-Q plot inmediatamente debajo del resultado SW
        ...
        fig_qq.update_layout(
            title="Grafico Q-Q Normal de residuales",
            xaxis_title="Cuantiles teoricos",
            yaxis_title="Cuantiles observados",
            height=370, margin=dict(t=50,b=50,l=60,r=20),
            legend=dict(orientation="h", y=1.12, x=0.5, xanchor="center"))
        st.plotly_chart(fig_qq, use_container_width=True)

    with col_bt:
        st.markdown("**Prueba de Bartlett** (homogeneidad de varianzas)")
        css_bt = "ok-box" if bt_ok else "err-box"
        msg_bt = ("Varianzas homogeneas (no se rechaza H0)"
                  if bt_ok else "Varianzas heterogeneas (se rechaza H0)")
        st.markdown(
            '<div class="' + css_bt + '">'
            'chi2 = ' + f'{r["bt_stat"]:.4f}' + ' | p = ' + f'{r["bt_p"]:.5f}' +
            ' | ' + msg_bt + '</div>',
            unsafe_allow_html=True)
        st.markdown(
            '<div class="theory-box" style="margin-top:.8rem">'
            '<b>Guia de interpretacion:</b><br><br>'
            '<b>Shapiro-Wilk + Q-Q:</b> evaluan la normalidad de los residuales. '
            'Si los puntos del Q-Q siguen la diagonal roja, la normalidad se cumple.<br><br>'
            '<b>Bartlett:</b> evalua si las varianzas dentro de cada unidad son iguales '
            '(homocedasticidad). Si se rechaza, considere la prueba de Levene '
            'como alternativa mas robusta.<br><br>'
            '<b>Si se violan ambos supuestos</b>, los resultados del ANOVA deben '
            'interpretarse con cautela. Considere una transformacion logaritmica '
            'o la prueba no parametrica de Kruskal-Wallis.'
            '</div>',
            unsafe_allow_html=True)
...
# Footer
st.markdown("---")
st.caption("ICA – Área IIAD · LANIA | Evaluación de homogeneidad de MR e ítems de EA | ISO 17034:2017 §9.4 | v2.5")
