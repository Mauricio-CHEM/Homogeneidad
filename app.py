...
# ════════════════════════════════════════════════════════════
st.set_page_config(
    page_title="Homogeneidad ISO 17034 · ICA",
...
.card{background:#f0f4f8;border-radius:10px;padding:1rem 1.4rem;margin-bottom:.8rem}
.theory-box{background:#eaf4fb;border-left:5px solid #2980b9;
...
.section-hdr{color:#1a5276;font-size:1.05rem;font-weight:600;
             border-bottom:2px solid #aed6f1;padding-bottom:3px;margin-top:1rem}
...
if pagina == "🏠 Inicio e Instrucciones":
    st.markdown('<p class="main-title">🧪 Evaluación de Homogeneidad de Materiales de Referencia</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-title">ISO 17034:2017 §9.4 · ISO Guide 35:2017 · ICA – Subgerencia de Análisis y Diagnóstico</p>', unsafe_allow_html=True)
...
"""
    ...
    fig_rv.add_trace(go.Scatter(x=r['fitted'], y=r['residuales'], mode="markers",
        marker=dict(size=9, color="#2980b9", opacity=0.7), showlegend=False))
...
# Footer
st.markdown("---")
st.caption("ICA – Subgerencia de Análisis y Diagnóstico | Evaluación de homogeneidad ISO 17034:2017 §9.4 | v2.0")
